...
namespace blockchain
{
   namespace net
   {
	  ...
	  memcpy(packet.mPrevHash, hashData, SHA256_DIGEST_LENTH);
	  delete[] hashData;
	  ...
	  if(recv(mSocket,(void*)&netNum,sizeof(uint32_t),0)<0)
		  throw std::runtime_error("Faild to receive UINT32");
	  return ntohl(netNum);
       }
	  ...
	  if(send(mSocket,(void*)&netNum,sizeof(uint32_t),0) < 0)
		  throw std::runtime_error("Faild to send UINT32");
       }

       int32_t INet::recvInt()
       {
	  uint32_t netNum = 0;
	  if(recv(mSocket,(void*)&netNum,sizeof(int32_t),0) < 0)
		  thhros std::runtime_error("Faild to receive INT32");
       }
	  ...
	  if(recv(mSocket, (void*)&netHigh, sizeof(uint32_t), 0) < 0)
		  throw std::runtime_error("Failed to receive INT64 (l)");
	  ...
	  return (((uint64_t)ntohl(netHigh) & 0xFFFFFFFFLL) << 32) | ((uint64_t)ntohl(netLow) >> 32);
	   ...
	   if(send(mSocket,(void*)&netLow,sizeof(uint32_t),0) < 0)
		   throw std::runtime_error("Failed to sendd UINT64(l)");
       }
